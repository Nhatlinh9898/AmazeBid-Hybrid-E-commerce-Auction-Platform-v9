...
export const StoreDiscovery: React.FC<StoreDiscoveryProps> = ({ onSelectStore, onViewMenu }) => {
  ...
  const categories = ['ALL', 'Food & Drink', 'Fashion', 'Retail', 'Beauty', 'Electronics', 'Furniture', 'Health', 'Education', 'Automotive', 'Grocery', 'Stationery', 'Sports', 'Services', 'Other'];
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case 'Retail': return <LucideStore className="w-4 h-4" />;
      case 'Beauty': return <Sparkles className="w-4 h-4" />;
      case 'Electronics': return <Laptop className="w-4 h-4" />;
      case 'Furniture': return <LucideStore className="w-4 h-4" />;
      case 'Health': return <LucideStore className="w-4 h-4" />;
      ...
      case 'Sports': return <LucideStore className="w-4 h-4" />;
  ...
  const getCategoryLabel = (category: string) => {
    switch (category) {
      ...
      case 'Automotive': return 'Ô tô & Xe máy';
      case 'Grocery': return 'Tạp hóa';
      case 'Stationery': return 'Văn phòng phẩm';
          ...
          return (
            ...
            >
              <div className="relative h-48 overflow-hidden">
                <img 
                  src={Array.isArray(store.images) && store.images.length > 0 ? store.images[0] : 'https://images.unsplash.com/photo-1541604193435-22587844b473?q=80&w=2070&auto=format&fit=crop'} 
                ...
                <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full flex items-center gap-2 shadow-sm">
                  <span className="text-blue-600">{getCategoryIcon(store.category || 'Other')}</span>
                  <span className="text-[10px] font-black uppercase tracking-wider">{getCategoryLabel(store.category || 'Other')}</span>
                </div>
                <div className="absolute bottom-4 right-4 bg-blue-600 text-white px-3 py-1 rounded-full flex items-center gap-1 shadow-lg">
                  ...
                  <span className="text-xs font-bold">{store.rating ?? 0}</span>
                </div>
              </div>
              
              <div className="p-5">
                <h3 className="text-xl font-bold text-gray-900 mb-2 group-hover:text-blue-600 transition-colors">{store.name || 'Untitled Store'}</h3>
                
                <div className="flex items-start gap-2 text-gray-500 text-sm mb-4">
                  <MapPin className="w-4 h-4 mt-0.5 shrink-0 text-red-400" />
                  <span className="line-clamp-1">{store.address || 'No address provided'}</span>
                </div>
                
                <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                  <div className="flex items-center gap-2 text-xs text-gray-400 font-medium">
                    <Clock className="w-3.5 h-3.5" />
                    {store.openingHours || 'Sắp mở cửa'}
                  </div>
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
